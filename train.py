...
tf.app.flags.DEFINE_string('save_dir', 'check_3_13/mnist_predrnn_pp',
                            'dir to store trained net.')
tf.app.flags.DEFINE_string('gen_frm_dir', 'res_3_13/mnist_predrnn_pp',
                           'dir to store result.')
# model
tf.app.flags.DEFINE_string('model_name', 'predrnn_pp',
                           'The name of the architecture.')
tf.app.flags.DEFINE_string('pretrained_model', 'split-predrnn-v1-pretrain/model.ckpt-60000',
                           'file of a pretrained model to initialize from.')
tf.app.flags.DEFINE_integer('input_length', 10,
                            'encoder hidden states.')
tf.app.flags.DEFINE_integer('seq_length', 30,   # 20 for train
...
#                            'batch size for training.')
tf.app.flags.DEFINE_integer('max_iterations', 1,   # 80000
                            'max num of steps.')
...
def main(argv=None):
    ...
    for itr in xrange(1, FLAGS.max_iterations + 1):
        ...
        if itr % FLAGS.test_interval == 0:
            ...
            avg_mse = 0
            batch_id = 0
            img_mse,ssim,psnr,fmae,sharp,ce = [],[],[],[],[],[]
            ...
            while(test_input_handle.no_batch_left() == False):
                ...
                for i in xrange(FLAGS.seq_length - FLAGS.input_length):
                    ...
                    ce[i] = metrics.batch_ce_frame_float(gx, x)
                    gx = np.maximum(gx, 0)
                    gx = np.minimum(gx, 1)
            ...
            ce = np.asarray(ce, dtype=np.float32) / batch_id
            ...
            for i in xrange(FLAGS.seq_length - FLAGS.input_length):
                print(sharp[i])
                f.write(' %f' % sharp[i])
            print('ce per frame: ' + str(np.mean(ce)))
            f.write('\nce per frame: %f' % np.mean(ce))
            for i in xrange(FLAGS.seq_length - FLAGS.input_length):
                print(ce[i])
                f.write(' %f' % ce[i])
